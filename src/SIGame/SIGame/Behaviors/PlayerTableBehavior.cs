...
namespace SIGame.Behaviors
{
    ...
    {
        public static GameSettingsViewModel GetIsAttached(DependencyObject obj)
        {
            return (GameSettingsViewModel)obj.GetValue(IsAttachedProperty);
        }
        ...
        private static void OnIsAttachedChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (settings != null)
            {
                ...
                if (index.HasValue)
                    radio.SetBinding(RadioButton.IsCheckedProperty, new Binding("PlayerNumber") { Source = settings, Converter = new EqualityConverter(), ConverterParameter = index.Value, Mode = BindingMode.TwoWay });
            }
            else
                BindingOperations.ClearBinding(radio, RadioButton.IsCheckedProperty);
        }
    }
}
